use std::net::TcpStream;
use std::sync::mpsc;
use std::sync::Arc;
use std::sync::Mutex;

use anyhow::Result;
use log::{debug, info, trace, warn};
use tungstenite::http::Response;
use tungstenite::protocol::WebSocketConfig;
use tungstenite::stream::MaybeTlsStream;
use url::Url;
// use crate::types::{parse_raw_message, Message};
#[derive(Debug)]
pub enum StatusMessage {
  Connected,
  Disconnected,
  ConnectionShutdown,
  MessageReceived(String),
  Error(String),
}

type TungsteniteWebsocketConnection = tungstenite::protocol::WebSocket<MaybeTlsStream<TcpStream>>;
const READ_TIMEOUT_DURATION: std::time::Duration = std::time::Duration::from_millis(30);

pub struct WebSocketConnection {
  connection: Arc<Mutex<TungsteniteWebsocketConnection>>,
  thread: std::thread::JoinHandle<()>,
  process_id: Option<u32>,
  // stop_flag: Arc<AtomicBool>,
}

// TODO websocket::sender::Writer is not :Debug...
impl std::fmt::Debug for WebSocketConnection {
  fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
    write!(f, "WebSocketConnection {{}}")
  }
}

impl WebSocketConnection {
  pub fn new(
    ws_url: &Url,
    process_id: Option<u32>,
    messages_tx: mpsc::Sender<StatusMessage>,
  ) -> Result<Self> {
    let (connection, _) = Self::websocket_connection(ws_url, messages_tx.clone())?;

    let connection = Arc::new(Mutex::new(connection));
    // let stop_flag = Arc::new(AtomicBool::new(false));
    let thread = {
      let sender = connection.clone();
      // let stop_flag = stop_flag.clone();
      std::thread::spawn(move || {
        trace!("Starting msg dispatching loop");
        Self::dispatch_incoming_messages(sender, messages_tx, process_id);
        trace!("Quit loop msg dispatching loop");
      })
    };

    Ok(Self {
      connection,
      thread,
      process_id,
    })
  }

  pub fn shutdown(&self) {
    trace!(
      "Shutting down WebSocket connection for Chrome {:?}",
      self.process_id
    );
    if let Err(err) = self.connection.lock().unwrap().close(None) {
      debug!(
        "Couldn't shut down WS connection for Chrome {:?}: {}",
        self.process_id, err
      );
    }

    self.connection.lock().unwrap().flush().ok();
    self.thread.thread().unpark();
  }

  fn dispatch_incoming_messages(
    receiver: Arc<Mutex<TungsteniteWebsocketConnection>>,
    messages_tx: mpsc::Sender<StatusMessage>,
    process_id: Option<u32>,
  ) {
    loop {
      let message = receiver.lock().unwrap().read();

      match message {
        Err(err) => match err {
          tungstenite::Error::Io(err) => {
            if matches!(
              err.kind(),
              std::io::ErrorKind::WouldBlock | std::io::ErrorKind::TimedOut
            ) {
              std::thread::park_timeout(READ_TIMEOUT_DURATION);
            } else {
              debug!("WS IO Error for Chrome #{:?}: {}", process_id, err);
              break;
            }
          }
          tungstenite::Error::ConnectionClosed
          | tungstenite::Error::AlreadyClosed
          | tungstenite::Error::Protocol(
            tungstenite::error::ProtocolError::ResetWithoutClosingHandshake,
          ) => break,
          error => {
            panic!("Unhandled WebSocket error for Chrome #{process_id:?}: {error:?}");
          }
        },
        Ok(message) => match message {
          tungstenite::protocol::Message::Text(message_string) => {
            if messages_tx
              .send(StatusMessage::MessageReceived(message_string.to_string()))
              .is_err()
            {
              break;
            }
          }
          tungstenite::protocol::Message::Binary(message_binary) => {
            let message_string = String::from_utf8_lossy(&message_binary);
            if messages_tx
              .send(StatusMessage::MessageReceived(message_string.to_string()))
              .is_err()
            {
              break;
            }
          }
          tungstenite::protocol::Message::Close(close_frame) => {
            match close_frame {
              Some(tungstenite::protocol::CloseFrame { code, reason }) => {
                debug!(
                  "Received close frame from Chrome #{:?}: {:?} {:?}",
                  process_id, code, reason
                );
                match code {
                  tungstenite::protocol::frame::coding::CloseCode::Normal => {
                    debug!("Normal close code, shutting down");
                  }
                  _ => {
                    panic!("Abnormal close code {code:?}, shutting down");
                  }
                }
              }
              None => {
                debug!("Received close frame from Chrome #{:?}: None", process_id);
              }
            }
            break;
          }
          tungstenite::protocol::Message::Ping(_ping_binary) => {
            debug!("Received Ping, sending Pong");
            //if let Err(e) = stream.write_message(tungstenite::protocol::Message::Pong(vec![])) {
            // let error = WebSocketError::WebSocketError(e);
            // error!("{}", error);
            // let _ = status_sender.send(StatusMessage::Error(error));
            //}
          }
          tungstenite::protocol::Message::Pong(pong_binary) => {
            // let unix_timestamp: i64 = Utc::now().timestamp();
            // timestamp.store(unix_timestamp as u64, Ordering::SeqCst);
            debug!("received pong = {:?}", pong_binary);
          }
          _ => {
            panic!("Got a weird message: {message:?}");
          }
        },
      }
    }

    info!("Sending shutdown message to message handling loop");
    if messages_tx.send(StatusMessage::ConnectionShutdown).is_err() {
      warn!("Couldn't send message to transport loop telling it to shut down");
    }
  }

  pub fn websocket_connection(
    ws_url: &Url,
    messages_tx: mpsc::Sender<StatusMessage>,
  ) -> Result<(
    tungstenite::WebSocket<MaybeTlsStream<TcpStream>>,
    Response<Option<Vec<u8>>>,
  )> {
    let mut config = WebSocketConfig::default();
    config.max_message_size = None;
    config.max_frame_size = None;
    config.accept_unmasked_frames = true;

    let mut client =
      tungstenite::client::connect_with_config(ws_url.as_str(), Some(config), u8::MAX - 1)?;

    let stream = client.0.get_mut();

    // this should be handled in tungstenite
    let stream = match stream {
      MaybeTlsStream::Plain(s) => s,
      // #[cfg(feature = "native-tls")]
      // MaybeTlsStream::NativeTls(s) => s.get_mut(),
      // #[cfg(feature = "rustls")]
      MaybeTlsStream::Rustls(s) => &mut s.sock,

      _ => todo!(),
    };
    stream.set_read_timeout(Some(READ_TIMEOUT_DURATION))?;

    debug!("Successfully connected to WebSocket: {}", ws_url);
    let _ = messages_tx.send(StatusMessage::Connected).is_err();

    Ok(client)
  }

  pub fn send_message(&self, message_text: &str) -> Result<()> {
    let message = tungstenite::protocol::Message::text(message_text);
    let mut sender = self.connection.lock().unwrap();
    sender.send(message)?;
    self.thread.thread().unpark();
    Ok(())
  }
}

impl Drop for WebSocketConnection {
  fn drop(&mut self) {
    info!("dropping websocket connection");
  }
}
